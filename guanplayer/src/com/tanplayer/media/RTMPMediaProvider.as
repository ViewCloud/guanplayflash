...
package com.tanplayer.media 
{
	...
	public class RTMPMediaProvider extends MediaProvider 
	{
		...
		public function onClientData(dat:Object):void 
		{
			...
			{
				trace("dat.bandwidth"+dat.bandwidth);
			...
			if (getConfigProperty('subscribe')) {
				...
				if (_currentFile != item.file) {
					...
					try 
					{
						...
						trace("getID(item.file)"+getID(item.file));
						_stream.play(getID(item.file));
					} catch(e:Error) {
		...
		private function setStream():void 
		{
			...
			trace("_timeoffset"+_timeoffset);
			
			this.dispatchEvent(new Event("ImdVideo"));
		}
		
		private function AutoConnect(str:String):void//自动重连
		{
			stop();//先停止
			_bufferFull=false;
			//因为无法从 netStatus 事件处理函数调用 NetConnection.connect。
			//所以做个延时
			var delay2:uint = flash.utils.setTimeout(function DelayConnect():void
			{
				flash.utils.clearTimeout(delay2);
				trace(str+"网络中断重连");
				_connection.connect(item.streamer);
			},1);
		}
		private var _notFoundNum:int=0;//找不到地址切换码流的次数
		private var currentNsState:String="publish";//当前流的状态
		/** Receive NetStream status updates. **/
		private function statusHandler(evt:NetStatusEvent):void 
		{
			trace("连接状态eeeexxxxxx：：："+evt.info.code);
			switch (evt.info.code) 
			{
				...
				case 'NetStream.Play.Start':
					sendMediaEvent(MediaEvent.JWPLAYER_MEDIA_START_PLAY);
					break;
				...
				case 'NetConnection.Connect.Failed'://流断了
					//2014725这里暂时不能加 一加就错
					//AutoConnect('NetConnection.Connect.Failed');
					//====以下是原版=====
					//stop();
					//sendMediaEvent(MediaEvent.JWPLAYER_MEDIA_ERROR, {message: "网络故障，请检查网络后刷新页面进行重试"}); // + item.streamer
					//=================
					break;
				case 'NetStream.Play.UnpublishNotify'://流发布方断了
					
					currentNsState="Unpublish";
					break;
				
				case 'NetStream.Play.PublishNotify':
					
					if(currentNsState=="Unpublish")//如果之前发布的流又恢复了自动重连
					{
						currentNsState="publish";
						AutoConnect('NetStream.Play.UnpublishNotify');
					}
					break;
				case 'NetStream.Buffer.Full'://如果之前发布的流又恢复了
					//跑完AutoConnect就回跑这条路
				...
				case 'NetConnection.Connect.Closed'://用户自己网络中断
					//目前只考虑直播
					//只要切换码流就会跑这一步
					//2014725这里暂时不能加 一加就错
					//AutoConnect('NetConnection.Connect.Closed');
					
					break;
				
				
				
				//				case 'NetStream.Buffer.Empty'://用户路由器重启或网线断
				//					
				//					pause();
				//					break;
			}
		...
		public function SwapStream(name:String):void
		{
			var nso:NetStreamPlayOptions = new NetStreamPlayOptions();
			nso.streamName = getID(name);
			nso.transition = NetStreamPlayTransitions.SWITCH;
			_stream.play2(nso);
		}
