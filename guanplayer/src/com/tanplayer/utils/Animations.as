package com.tanplayer.utils {
	...
	import com.tanplayer.view.PlayerComponents;
	...
	public class Animations
	{
		...
		private function fadeHandler(evt:Event):void 
		{
			if ((_tgt.alpha >= _end - _spd && _spd > 0) || (_tgt.alpha <= _end + _spd && _spd < 0)) 
			{
				...
				if (_end == 0) 
				{
					if(_tgt is ControlbarComponentV4)
					{
					
						//全屏隐藏
						_tgt.imdPlayer.view.components.display.clearDisplay();
			...
			else 
			{
				...
				{
					//全屏显示
					//RootReference.ServiceChoiceShow();
