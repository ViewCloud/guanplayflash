...
package com.tanplayer.model {
	...
	import com.tanplayer.controller.RepeatOptions;
	import com.tanplayer.player.PlayerVersion;
	...
	import com.tanplayer.utils.TypeChecker;
	...
	public dynamic class PlayerConfig extends EventDispatcher 
	{
		...
		//protected var _bufferlength:Number 	= 0.00001; //缓冲长度
		protected var _bufferlength:Number 	= 3; //缓冲长度
		...
		protected var _showtanmu:Boolean   =true;//是否显示弹幕
		protected var _havead:Boolean=false;//是否有前置广告
		
		//TODO: Move to ENUM class
		protected var _stretching:String 	= "uniform"; 
		protected var _volume:Number 		= 100;//初始化音量
		...
		public function get showtanmu():Boolean
		{
			return _showtanmu;
		}
		
		public function set showtanmu(val:Boolean):void
		{
			_showtanmu=val;
		}
		
		private var _isvr:Boolean;//是否是360视频
		...
		public function set isvr(val:Boolean):void
		{
			_isvr=val;
		}
		...
		public function set havead(val:Boolean):void
		{
			_havead=val;
		}
		
		private var _playType:String;
		private var _reportPlayTime:String;
		...
		public function set playtype(val:String):void
		{
			_playType=val;
		}
		
		//注意这里不要用大写
		public function get reportplaytime():String
		{
			return _reportPlayTime;
		}
		
		public function set reportplaytime(val:String):void
		{
			_reportPlayTime=val;
		}
		...
		private var _frontadurl:String;//前置广告地址
		
		public function get frontadurl():String
		{
			return _frontadurl;
		}
		
		public function set frontadurl(val:String):void
		{
			_frontadurl=val;
		}
		...
		private var _adtime:int;//前置广告时长
		
		public function get adtime():*
		{
			return _adtime;
		}
		
		public function set adtime(val:String):void
		{
			_adtime=int(val);
		}
		...
		public function setConfig(config:Object):void 
		{
			...
			for (var item:String in config) 
			{
				...
				{
					if (item == "file" && Strings.extension(config[item]) == "xml")//播放列表 
					{
						setProperty("playlistfile", config[item]);					
					} 
					else if(item=="applehttp")//如果有苹果流
					{
						_singleItem["applehttp"]=config[item];
					}
					else //只有一个文件
					{
						trace("item"+item);
						trace("config[item]"+config[item]);
						
						if((config[item] as String).indexOf("rtmp")!=-1)//根据判断走rtmp
						{
							var num:int = (config[item] as String).lastIndexOf("/");
							//去掉了这里所以加的
							//so.addVariable('file','');//注意这里必须有这两个参数为空就行
							//so.addVariable('streamer','');
							_singleItem.file =(config[item] as String).slice((num+1));
							_singleItem.streamer = (config[item] as String).slice(0,num);
						}
						else//走http流
						{
		...
		{
			trace("flash变量name：：："+name);
			trace("flash变量value：：："+value);
			
			if (hasOwnProperty(name)) 
			{
				try 
				{
					this[name] = TypeChecker.fromString(value, TypeChecker.getType(this, name));//将字符串转布尔值
				} 
				catch (e:Error) 
				{
