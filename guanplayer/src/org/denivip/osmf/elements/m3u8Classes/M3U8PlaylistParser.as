	...
	public class M3U8PlaylistParser extends EventDispatcher
	{
		...
		public function parse(value:String, baseResource:URLResource):void{
			...
			for(var i:int = 1; i < lines.length; i++){
				...
				//colowap 这里要注意下变量定义重复temp_res改成temp_res3
				if(line.indexOf("#EXT-X-STREAM-INF:") == 0){
					if(!result){
						result = new HLSDynamicStreamingResource(baseResource.url);
						tempDynamicRes = result as DynamicStreamingResource;
						var temp_res3:StreamingURLResource = (baseResource as StreamingURLResource);
						if(!temp_res3)
							temp_res3 = new StreamingURLResource(baseResource.url);
						
						tempStreamingRes = HLSStreamingResource.createHLSResource(temp_res3);
						if(tempStreamingRes){
			...
			//colowap 这里要注意下变量定义重复temp_res改成temp_res2
