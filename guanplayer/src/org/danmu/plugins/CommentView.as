...
//            clipMask.name = 'commentviewmasklayer';
        ...
        public function initPlugin(ply:com.tanplayer.player.IPlayer, conf:com.tanplayer.plugins.PluginConfig):void
        {
            player = ply;
            config = conf;
...
//            _root.addChildAt(clipMask,_root.getChildIndex(_masked) + 1);
            _root.addChildAt(_clip,_root.numChildren-1);
//            clip.mask = clipMask;
		...
		public function ShowOrHideTanmu(bo:Boolean):void
		{
			_clip.visible =bo;
		}
        ...
        private function setUpRightClick(_root:MovieClip):void
        {
            ...
            aboutMenuItem = new ContextMenuItem("关于 " + _version + '...');
        ...
        {
            var cb:com.tanplayer.interfaces.IControlbarComponent = player.controls.controlbar;
...
//            cb.addButton(_visibleButtonIcon,'VisibleBt',visibleButtonHandler);
        ...
        **/
        ...
        /** 状态改变事件监听器,监听暂停或者播放 **/
        private function stateHandler(event:com.tanplayer.events.PlayerStateEvent):void
        {
            var i:int;
            var c:DisplayObject;
...
//            _innerFullScreenButton.visible = stage.displayState == 'fullScreen' ? false:true;
        ...
        * 弹幕可能滞后,因为没有同步加载
        **/
        private function itemHandler(event:com.tanplayer.events.PlaylistEvent):void
        {
            //不再使用,加载弹幕由主程序控制
...
//            }
        ...
        * 播放时间事件
        **/
        private function timeHandler(event:com.tanplayer.events.MediaEvent):void
        {
